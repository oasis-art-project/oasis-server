...
class EventSchema(BaseSchema):
    ...
    def get(self, data):
        if 'place' in data:
            ...
            if not place:
                raise ValueError
            data['place'] = PlaceSchema(only=('id', 'tags', 'host', 'name', 'description', 'address', 'latitude', 'longitude')).dump(place)
        ...
        if 'artists' in data:
            for index in range(len(data['artists'])):
                ...
                if not artist:
                    raise ValueError
                data['artists'][index] = UserSchema(only=('id', 'tags', 'firstName', 'lastName', 'bio', 'twitter', 'flickr', 'instagram')).dump(artist)
